...
RUN cd /opt/ \
    ...
    && cd nginx-1.18.0 \
    && ./configure --user=nginx --group=nginx --prefix=/usr/local/nginx --with-http_ssl_module --with-http_stub_status_module --with-http_realip_module --with-threads --with-file-aio --add-module=/opt/headers-more-nginx-module-master \
